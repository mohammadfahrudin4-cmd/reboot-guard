...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        addSection("② 高风险时段");
        addText("使用 24 小时制，例如 22:30 到 01:00。跨午夜可以正常识别。", 13, Color.GRAY);
        ...
        startTime.setText(prefs.getString("riskStart", "22:30"));
        endTime.setText(prefs.getString("riskEnd", "01:00"));
        timeRow.addView(startTime, new LinearLayout.LayoutParams(0, dp(52), 1));
        Space s = new Space(this);
        timeRow.addView(s, new LinearLayout.LayoutParams(dp(10), 1));
        timeRow.addView(endTime, new LinearLayout.LayoutParams(0, dp(52), 1));
        root.addView(timeRow);

        addSection("③ 选择高风险 App");
        addText("勾选你想让「重启防线」在危险时段主动干预的应用。", 13, Color.GRAY);
        loadLauncherApps();
        ...
        addButton("测试一次全屏干预", v -> {
            ...
            i.setAction(GuardService.ACTION_TEST);
    ...
    private void loadLauncherApps() {
        ...
        infos.sort((a, b) -> collator.compare(
                String.valueOf(a.loadLabel(pm)),
                String.valueOf(b.loadLabel(pm))
        ));

        Set<String> selected = prefs.getStringSet("blockedApps", new HashSet<>());

        for (ResolveInfo info : infos) {
            String pkg = info.activityInfo.packageName;
            if (pkg.equals(getPackageName()) || appBoxes.containsKey(pkg)) continue;
    ...
    private void saveSettings() {
        ...
        for (Map.Entry<String, CheckBox> e : appBoxes.entrySet()) {
            if (e.getValue().isChecked()) selected.add(e.getKey());
        }

        prefs.edit()
                .putString("riskStart", st)
                .putString("riskEnd", et)
                .putStringSet("blockedApps", selected)
                .apply();

        startTime.setText(st);
        endTime.setText(et);
        Toast.makeText(this, "已保存：" + selected.size() + " 个高风险 App", Toast.LENGTH_SHORT).show();
    }
    ...
    private void startGuard() {
        ...
        if (!hasUsageAccess()) {
            Toast.makeText(this, "还没有授予「使用情况访问」", Toast.LENGTH_LONG).show();
            startActivity(new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS));
            return;
        }
        if (!Settings.canDrawOverlays(this)) {
            Toast.makeText(this, "还没有授予悬浮层权限", Toast.LENGTH_LONG).show();
    ...
    private void addSection(String title) {
        TextView tv = addText(title, 18, Color.rgb(17, 24, 39));
    ...
    private void addTitle(String title) {
        TextView tv = addText(title, 30, Color.rgb(17, 24, 39));
        tv.setTypeface(null, 1);
    }

    private TextView addText(String text, int sp, int color) {
